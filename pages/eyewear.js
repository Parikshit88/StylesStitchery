...
const Eyewear = ({ products }) => {
  return (
    <div>
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-16 mx-auto">
          <div className="flex flex-wrap -m-4">
            {Object.keys(products).length ===0 && <p>Eyewear Out Of Stock. New Stock coming soon. Stay Tuned!</p> }
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({ category: "eyewear" });
  let eyewear = {};
  for (let item of products) {
    if (item.title in eyewear) {
      if (
        !eyewear[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        eyewear[item.title].color.push(item.color);
      }
      if (
        !eyewear[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
        eyewear[item.title].size.push(item.size);
      }
    } else {
      eyewear[item.title] = JSON.parse(JSON.stringify(item));
      if (item.availableQty > 0) {
        eyewear[item.title].color = [item.color];
        eyewear[item.title].size = [item.size];
      }
    }
  }
  return {
    props: { products: JSON.parse(JSON.stringify(eyewear)) },
  };
}
