      ...
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-16 mx-auto">
          <div className="flex flex-wrap -m-4">
            {Object.keys(products).length ===0 && <p>Footwear Out Of Stock. New Stock coming soon. Stay Tuned!</p> }
...
export async function getServerSideProps(context) {
  ...
  for (let item of products) {
    if (item.title in footwear) {
      if (
        !footwear[item.title].color.includes(item.color) &&
        item.availableQty > 0
      ) {
        footwear[item.title].color.push(item.color);
      }
      if (
        !footwear[item.title].size.includes(item.size) &&
        item.availableQty > 0
      ) {
  ...
  return {
    props: { products: JSON.parse(JSON.stringify(footwear)) },
  };
}
